...
class OrdenesController extends Controller
{
	...
	{
		return $this->render($response, 'App.Ordenes.Ordenes', [
			'titulo_web' => 'Ordenes',
			'url' => $request->getUri()->getPath(),
			'permisos' => $this->permisos,
			'js' => ['js/app/nw_ordenes.js'],
			'tk' => [
	...
	public function store($request, $response)
	{
		...
		*/
		...
		$rq = $model->create([
			'or_nombre' => ucfirst($data['or_nombre']),
	...
	{
		if (empty($data['or_nombre'])) {
			return false;
		}
		// if (empty($data['or_descripcion'])) {
		// 	return false;
		// }
		return true;
	}
